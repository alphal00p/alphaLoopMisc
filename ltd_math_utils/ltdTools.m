...
contractLorentz::usage="Contracts Lorentz-indices and Spinor indices and performs Gamma-traces.: 
	 It works with the following objects:
		...
		-------------------------------------------------------------------
		Spinors:
		spinorUbar[{momentum,mass},spinorIndex]
		spinorVbar[{momentum,mass},spinorIndex]
		spinorU[{momentum,mass},spinorIndex]
		spinorV[{momentum,mass},spinorIndex]
	INPUT:
...
(* anticommutation until canonical ordering is achieved *)
	...
	antiCom[lVec[sInd2_],sInd1_]:=2 vector[sInd2,sInd1];
	antiCom[lVec[sInd1_],lVec[sInd2_]]:=2 SP[sInd2,sInd1];
	antiCom[sInd1_,sInd2_]/;Head[sInd1]=!=lVec&&Head[sInd2]=!=lVec:=2 g[sInd2,sInd1];
	sym[lVec[sInd1_],lVec[sInd1_]]:=SP[sInd1,sInd1];
	sym[sInd1_,sInd1_]/;Head[sInd1]=!=lVec:=d;
...
rulesForContraction=Dispatch@{
	...
	delta[sunF[a_],sunF[b_]]T[c__]/;(!FreeQ[{c},a]&&!StringMatchQ[ToString@a,ToString@b]):>ReplaceAll[T[c],a->b],
...
(*	     Print[ind];
	     Pause[.2];*)
	     replV=vector[x_,lMomInd[loopMomNum_][loopIndex_]]:>vector[ x,ind[[loopMomNum,loopIndex]]];
		 ...
		 replGamma=gamma[x___,lMomInd[loopMomNum_][loopIndex_],y___]:>gamma[x,ind[[loopMomNum,loopIndex]],y];
	     resTmp //. replV //. replG //. replGamma
	    ,{ind,myInd}];
