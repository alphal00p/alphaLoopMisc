...
GLOBAL_SORT_ANCHOR_SQRT_S = 2*173.0+2 - 2*173.0 #(2*173.0+1)-2*173.0 # e.g., 1600.0 or None (for mean sort)
SQRT_S_MIN_DISPLAY = 0.1
SQRT_S_MAX_DISPLAY = None#9001.0
...
def generate_mock_raw_data_file(filename=raw_data_filename, N_supergraphs=124, N_energies=32):
...
# --- Call once if needed ---
## import os
# if not os.path.exists(raw_data_filename): # Generate only if it doesn't exist
...
try:
    ...
    print(f"Error loading or parsing data from {raw_data_filename}: {e}")
    sys.exit(1)
...
if SORT_MODE == 'global':
    if GLOBAL_SORT_ANCHOR_SQRT_S is not None and N_energies_loaded > 0:
        ...
        print("ANCHOR AT ",actual_anchor_s_val_for_title, "ID=",anchor_idx)
...
for j in range(N_energies_loaded): # For each energy column
    ...
    # VH fix, actually normalize to the maximal absolute value contrib always
    normalizer_neg = max(abs(normalizer_neg),normalizer_pos)
    normalizer_pos = max(abs(normalizer_neg),normalizer_pos)
    ...
    for j in range(N_energies_loaded): # For each energy point
        ...
        for p_val in TOP_PERCENTILES_TO_PLOT:
            ...
            if np.abs(total_cross_section[j]) > small_total_xs_threshold_for_ratio:
                rel_contrib_top_percentiles[p_val][j] = abs(1.-sum_contrib_top_percentiles[p_val][j] / total_cross_section[j])
...
ax_heatmap.legend(fontsize=FONT_SIZE_LEGEND)
ax_lineplot.legend(fontsize=FONT_SIZE_LEGEND)
ax_percent_contrib.legend(fontsize=FONT_SIZE_LEGEND)
ax_total_xs_loglog.legend(fontsize=FONT_SIZE_LEGEND)

ax_lineplot.set_ylim([0.1, 300.01])
ax_percent_contrib.set_ylim([8.0e-4, 20.01])
ax_total_xs_loglog.set_ylim([1.0e-4, 1.01])
...
heatmap_title_str = r'Individual forward scattering graph contributions at $\mu_r = m_Z$ as a function of $\sqrt{s}$' # Base title
heatmap_ylabel_str = ''
ytick_positions = []
...
heatmap_ylabel_str = "Forward scattering graphs"
ax_heatmap.set_ylabel(heatmap_ylabel_str, fontsize=FONT_SIZE_AXIS)
...
heatmap_title_str = r'Individual forward scattering graph contributions as a function of $\sqrt{s}$' # Base title
ax_heatmap.set_title(heatmap_title_str, fontsize=FONT_SIZE_TITLE)
...
rel_sum_positive = rel_sum_positive+rel_sum_negative
rel_sum_positive -= 1.0
ax_lineplot.set_yscale('log')
ax_lineplot.plot(sqrt_s_values_loaded, rel_sum_positive, label='1 - sum of relative absolute value contributions', color='darkgreen', linestyle='-', lw=2)
#ax_lineplot.plot(sqrt_s_values_loaded, rel_sum_negative, label='1-Sum of relative negative contributions', color='royalblue', linestyle='-', lw=2)
...
ax_lineplot.set_ylabel('Relative aggregate contributions', fontsize=FONT_SIZE_AXIS)
ax_lineplot.grid(True, which='both', linestyle=':', linewidth=0.7)
ax_lineplot.legend(loc='best', fontsize=FONT_SIZE_TICKS)
ax_lineplot.set_title('Relative aggregate contributions & cancellations', fontsize=FONT_SIZE_SUBTITLE)
...
if N_supergraphs_loaded > 0: # Only plot if there are supergraphs
    for i, p_val in enumerate(TOP_PERCENTILES_TO_PLOT):
        label = f'1 - sum of top {int(p_val*100)}% relative contributions' # e.g., "Sum Top 10% / Total"
...
ax_percent_contrib.set_yscale('log')
#ax_percent_contrib.axhline(1.0, label='Sum of all relative contributions := 1', color='black', linestyle='-', linewidth=1.5) # Line at y=1 for reference
ax_percent_contrib.set_ylabel('Relative aggr. contribution [-]', fontsize=FONT_SIZE_AXIS)
ax_percent_contrib.grid(True, which='both', linestyle=':', linewidth=0.7)
ax_percent_contrib.legend(loc='best', fontsize=FONT_SIZE_TICKS)
ax_percent_contrib.set_title(r'Abs. value of relative contribution from percentiles of forward scattering graphs (ranked per $\sqrt{s}$ by magnitude)', fontsize=FONT_SIZE_SUBTITLE)
...
# (2) Find the first index that is masked in 'total_xs_positive_part_masked'.
#     total_xs_positive_part_masked.mask is a boolean array; True means “this entry is masked.”
JOIN_LEFT = False
if JOIN_LEFT:
    masked_indices = np.flatnonzero(total_xs_positive_part_masked.mask)
    if masked_indices.size == 0:
        raise ValueError("No masked entries found in total_xs_positive_part_masked.")
    first_masked_idx = masked_indices[0]

    # (3) Find the first index that is *not* masked in 'total_xs_negative_part_abs_masked'.
    #     total_xs_negative_part_abs_masked.mask is True where data is masked, so ~total_xs_negative_part_abs_masked.mask is where it is valid.
    unmasked_neg_indices = np.flatnonzero(~total_xs_negative_part_abs_masked.mask)
    if unmasked_neg_indices.size != 0:
        first_unmasked_neg_idx = unmasked_neg_indices[0]

        # (4) Extract that numerical value from total_xs_negative_part_abs_masked:
        value_to_copy = total_xs_negative_part_abs_masked[first_unmasked_neg_idx]
        # Note: indexing a masked array at an unmasked index returns a plain Python scalar (float).

        # (5) Assign it into the masked slot of total_xs_positive_part_masked. This assignment will
        #     automatically clear the mask at that position, so it becomes a real data point:
        total_xs_positive_part_masked[first_masked_idx] = value_to_copy
else:
    masked_indices = np.flatnonzero(total_xs_negative_part_abs_masked.mask)
    if masked_indices.size == 0:
        raise ValueError("No masked entries found in total_xs_positive_part_masked.")
    last_masked_idx = masked_indices[-1]

    # (3) Find the first index that is *not* masked in 'total_xs_negative_part_abs_masked'.
    #     total_xs_negative_part_abs_masked.mask is True where data is masked, so ~total_xs_negative_part_abs_masked.mask is where it is valid.
    unmasked_pos_indices = np.flatnonzero(~total_xs_positive_part_masked.mask)
    if unmasked_pos_indices.size != 0:
        last_unmasked_pos_idx = unmasked_pos_indices[-1]

        # (4) Extract that numerical value from total_xs_negative_part_abs_masked:
        value_to_copy = total_xs_positive_part_masked[last_unmasked_pos_idx]
        # Note: indexing a masked array at an unmasked index returns a plain Python scalar (float).

        # (5) Assign it into the masked slot of total_xs_positive_part_masked. This assignment will
        #     automatically clear the mask at that position, so it becomes a real data point:
        total_xs_negative_part_abs_masked[last_masked_idx] = value_to_copy


if has_plottable_positive_total_xs:
    ax_total_xs_loglog.plot(sqrt_s_values_loaded, total_xs_positive_part_masked, label='$+\Delta\sigma^{(\mathrm{NNLO~QCD})}(\mu_r = m_Z)$', color='darkgreen', linestyle='-', lw=2)
if has_plottable_negative_total_xs: # If there are significant negative contributions
    ax_total_xs_loglog.plot(sqrt_s_values_loaded, total_xs_negative_part_abs_masked, label='$-\Delta\sigma^{(\mathrm{NNLO~QCD})}(\mu_r = m_Z)$', color='darkred', linestyle='--', lw=2)

ax_total_xs_loglog.set_xscale('log')
ax_total_xs_loglog.set_yscale('log')
ax_total_xs_loglog.set_xlabel(r'$\sqrt{s}-2 m_t$ (GeV)', fontsize=FONT_SIZE_AXIS)
ax_total_xs_loglog.set_ylabel('$\Delta\sigma^{(\mathrm{NNLO~QCD})}$ [pb]', fontsize=FONT_SIZE_AXIS) # Adjust unit if known
if has_plottable_positive_total_xs or has_plottable_negative_total_xs: # Show legend only if something is plotted
    ax_total_xs_loglog.legend(loc='best', fontsize=FONT_SIZE_TICKS)
ax_total_xs_loglog.grid(True, which='both', linestyle=':', linewidth=0.7)
ax_total_xs_loglog.set_title('$\Delta\sigma^{(\mathrm{NNLO~QCD})}$', fontsize=FONT_SIZE_SUBTITLE)
...
if N_energies_loaded > 0 :
    ...
    if X_MIN is not None and X_MAX is not None:
        ...
        ax_lineplot.set_xlim([X_MIN, X_MAX])
        ax_percent_contrib.set_xlim([X_MIN, X_MAX])
        ax_total_xs_loglog.set_xlim([X_MIN, X_MAX])
...
#fig.suptitle(fr'$\gamma\gamma \to t\bar{{t}}$ Forward scattering graph contributions', fontsize=18)
...
plt.savefig(f"supergraph_hierarchy.png", dpi=300)
