...
# anchor slice (None to integrate across range)
GLOBAL_SORT_ANCHOR_SQRT_S = 500.0
...
X_MIN_SINGLET, X_MAX_SINGLET = X_MIN, X_MAX
X_MIN_SINGLET, X_MAX_SINGLET = 5., 9001.0
...
HEATMAP_GLOBAL_SORT_Y_LABELS = [
    ...
    'GL94', 'GL222', 'GL372', 'GL402', 'GL218', 'GL168', 'GL172', 'GL208', 'GL380', '-AA_GG'
]
HEATMAP_TICKS_TO_HIGHLIGHT = ['GL168', 'GL172', 'GL208', 'GL380', '-AA_GG']
HEATMAP_ALL_GRAPHS_FONTSIZE = 7
HEATMAP_COLOR_SCALE = 'pow'  # 'linear','log','pow'
HEATMAP_LOG_LINTHRESH = 1.0e-2
HEATMAP_COLOR_POWER_EXPONENT = 0.5
MIN_ROW_WIDTH, MAX_ROW_WIDTH = 1.0, 6.0
...
def load_data(filename):
    ...
    entries = [e for e in raw if SQRT_S_MIN_DISPLAY <=
               e[0] <= (SQRT_S_MAX_DISPLAY or e[0])]
    ...
    for j, (_, graphs) in enumerate(entries):
        for i, (nm, val_err) in enumerate(sorted(graphs, key=lambda x: x[0])):
            val, err = (val_err if isinstance(
                val_err, (list, tuple)) else (val_err, 0.0))
...
def compute_row_edges(strength):
    ...
    norm_s = (strength - smin) / \
        (smax - smin) if smax > smin else np.ones_like(strength)
...
def main(raw_file):
    ...
    if os.path.exists('NLO_xsec.py'):
        ...
        raw_nlo = [e for e in raw_nlo if SQRT_S_MIN_DISPLAY <=
                   e[0] <= (SQRT_S_MAX_DISPLAY or e[0])]
    ...
    if os.path.exists('LO_xsec.py'):
        ...
        raw_lo = [e for e in raw_lo if SQRT_S_MIN_DISPLAY <=
                  e[0] <= (SQRT_S_MAX_DISPLAY or e[0])]
        s_lo = np.array([e[0] for e in raw_lo])
        xs_lo = np.array([e[1] for e in raw_lo])

    # --- Optional singlet total x-sec ---
    s_singlet = xs_singlet = None
    if os.path.exists('singlet_xsec.py'):
        raw_singlet = sorted(
            eval(open('singlet_xsec.py').read()), key=lambda x: x[0])
        raw_singlet = [e for e in raw_singlet if SQRT_S_MIN_DISPLAY <=
                       e[0] <= (SQRT_S_MAX_DISPLAY or e[0])]
        s_singlet = np.array([e[0] for e in raw_singlet])
        xs_singlet = np.array([e[1][0] for e in raw_singlet])
        xs_singlet_err = np.array([e[1][1] for e in raw_singlet])
    ...
    if HEATMAP_COLOR_SCALE == 'pow':
        disp_plot = np.sign(disp_norm) * (np.abs(disp_norm)
                                          ** HEATMAP_COLOR_POWER_EXPONENT)
        norm = mcolors.Normalize(-1, 1)
    ...
    rel_pos = np.divide(sum_pos, total_xs, out=np.zeros_like(
        sum_pos), where=np.abs(total_xs) > 1e-15)
    rel_neg = np.divide(sum_neg, total_xs, out=np.zeros_like(
        sum_neg), where=np.abs(total_xs) > 1e-15)
    rel_agg = np.abs(rel_pos) + np.abs(rel_neg) - 1.0

    sum_contrib_top = {p: np.zeros_like(s_vals)
                       for p in TOP_PERCENTILES_TO_PLOT}
    rel_contrib_top = {p: np.zeros_like(s_vals)
                       for p in TOP_PERCENTILES_TO_PLOT}
    ...
    fig.canvas.draw()
    hb = ax_hm.get_position()
    left, width = hb.x0, hb.width
    ...
    if HEATMAP_GLOBAL_SORT_Y_LABELS is None:
        ...
        sel = [names_ord.index(
            nm) for nm in HEATMAP_GLOBAL_SORT_Y_LABELS if nm in names_ord]
        ticks = y_mid[sel]
        labels = [names_ord[i] for i in sel]
    ...
    for label in ax_hm.get_yticklabels():
        txt = label.get_text()
        if txt in HEATMAP_TICKS_TO_HIGHLIGHT:
            label.set_color("red")
    ...
    ax_line.set_ylabel('Relative aggregate contributions',
    ...
    ax_line.set_title(
        'Relative aggregate contributions & cancellations', fontsize=FONT_SIZE_SUBTITLE)
    ax_line.set_xlabel('')
    ...
    for i, p in enumerate(TOP_PERCENTILES_TO_PLOT):
        if not USE_ABS_CONTRIBS_FOR_PERCENTILE_PLOT:
            ax_pct.plot(
                ...
                label=f'abs(1 - sum of top {int(p*100)
            ...
            ax_pct.plot(
                s_vals, rel_contrib_top[p],
                label=f'(sum of top {
                    int(p*100)}% relative abs. contributions)) / {p}',
                color=palette[i % len(palette)], lw=2, marker=MARKER_PERCENT, markersize=MARKERSIZE_PERCENT
            )

    ax_pct.set_ylabel(
        'Relative aggr. contribution [-]', fontsize=FONT_SIZE_AXIS)
    ...
    if has_pos_nnlo:
        ...
        ax_tot.plot(s_vals, tp_nnlo, label=lbl, color='darkgreen',
                    lw=2, marker=MARKER_TOTAL, markersize=MARKERSIZE_TOTAL)
    ...
    if s_nlo is not None:
        ...
        if has_pos_nlo:
            ...
            ax_tot.plot(s_nlo, tp_nlo, label=lbl, color='blue', lw=2)
        if has_neg_nlo:
    ...
    # singlet
    if s_singlet is not None:
        mask_singlet = (s_singlet >= X_MIN_SINGLET) & (
            s_singlet <= X_MAX_SINGLET)
        xs_singlet = np.ma.masked_where(~mask_singlet, xs_singlet)
        xs_singlet_err = np.ma.masked_where(~mask_singlet, xs_singlet_err)
        # xs_singlet = xs_singlet[mask_singlet]
        # xs_singlet_err = xs_singlet_err[mask_singlet]
        tp_singlet, tn_singlet, tp_singlet_err, tn_singlet_err = glue_sign_flips(
            xs_singlet, xs_singlet_err)
        has_pos_singlet = np.any(tp_singlet[mask_singlet] > THRESHOLD)
        has_neg_singlet = np.any(tn_singlet[mask_singlet] > THRESHOLD)
        if has_pos_singlet:
            lbl = (r'$\Delta\sigma^{(\mathrm{singlet})}$' if not has_neg_singlet
                   else r'$+\Delta\sigma^{(\mathrm{singlet})}$')
            ax_tot.plot(s_singlet, tp_singlet, label=lbl,
                        color='lightgreen', lw=2)
            ax_tot.errorbar(s_singlet, tp_singlet, yerr=tp_singlet_err,
                            fmt='none', ecolor='gray', elinewidth=1, capsize=2)

        if has_neg_singlet:
            ax_tot.plot(s_singlet, tn_singlet,
                        label=r'$-\Delta\sigma^{(\mathrm{singlet})}$',
                        color='lightgreen', lw=2, ls='--')
            ax_tot.errorbar(s_vals, tn_singlet, yerr=tn_singlet_err,
                            fmt='none', ecolor='gray', elinewidth=1, capsize=2)
    ...
    if (s_nlo is not None) and (s_lo is not None):
        if np.array_equal(s_nlo, s_lo):
            ...
            raise BaseException("Expected same shape NLO")
            s_tot = s_nlo
            xs_lo_i = np.interp(s_nlo, s_lo, xs_lo)
            xs_tot = xs_nlo + xs_lo_i

        xs_tot = xs_tot + total_xs
        ...
        if has_neg_tot:
            ax_tot.plot(s_tot, tn_tot, label='-Total',
                        color='k', lw=2, ls='--')

    ax_tot.set_ylabel(
        r'$\Delta\sigma^{(\mathrm{NNLO~QCD})}$ [pb]', fontsize=FONT_SIZE_AXIS)
