...
c HSS DEBUG
      ...
      ybst_til_tolab=-ycm_cnt(0)-0.5d0*dlog(ebeam(1)/ebeam(2))
...
c     xlum(kf)=photonpdfsquare(bjx(1,i),bjx(2,i))
c     HSS DEBUG (for electron beam)
c     we take q2max=1d0
                     xlum(kf)=1d0
                     xlum(kf)=xlum(kf)*epa_electron(bjx(1,i),1d0)
                     xlum(kf)=xlum(kf)*epa_electron(bjx(2,i),1d0)
                  else
                     xlum(kf)=1d0
                     LP=SIGN(1,LPP(1))
                     pd=pdg(1,i)
...
c call the PDFs
               if (lpp(1).eq.2 .and. lpp(2).eq.2) then
c     xlum=photonpdfsquare(bjx(1,i),bjx(2,i))
c     HSS DEBUG (for electron beam)
c     we take q2max=1d0
                  xlum=1d0
                  xlum=xlum*epa_electron(bjx(1,i),1d0)
                  xlum=xlum*epa_electron(bjx(2,i),1d0)
               else
                  xlum=1d0
                  LP=SIGN(1,LPP(1))
                  pd=pdg(1,i)
