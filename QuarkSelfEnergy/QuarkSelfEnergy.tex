...
\section{Explicit expression of $\Delta \textrm{UV}$}
\label{sec:explicitUV}
...
&{\Bigg ] }&+\Delta \textrm{UV}_m + (p^0-\sqrt{|\vec{p}|+m^2})\Delta \textrm{UV}_\psi\nonumber 
\end{eqnarray}
Then, writing $\Sigma_{\textrm{LTD}}\equiv \Sigma(p)-\left( \int d^{4-2\epsilon}k\; \textrm{UVCT}(k){\Big |}_{d=4} \right)$ and simply imposing Eqs.~\eqref{onshell2} yields the defining expressions for our terms $\Delta \textrm{UV}_m$ and $\Delta \textrm{UV}_\psi$ ( e already took $m_p=m_r\equiv m$):
\begin{eqnarray}
\Sigma_R(p^2=m_\text{p}^2) = 0 &=&
\Sigma_\textrm{LTD}(m_\text{p}^2)+m_\text{r} \delta Z_m
+ \left( \int d^{4-2\epsilon}k\; \textrm{UVCT}(k,p^2=m^2){\Big |}_{d=4-2\epsilon} \right) \nonumber\\
&-&\textrm{R-terms}(p^2=m^2) + \Delta \textrm{UV}_m \\
\Delta\textrm{UV}_\psi &=&\textrm{TODO} 
\end{eqnarray}
And when using $\Sigma_\textrm{LTD}(m_\text{p}^2)=0$ we find:
\begin{eqnarray}
\Delta \textrm{UV}_m &=& \textrm{R-terms}(p^2=m^2)  -m_\text{r} \delta Z_m -  \left( \int d^{4-2\epsilon}k\; \textrm{UVCT}(k,p^2=m^2){\Big |}_{d=4-2\epsilon} \right) \\ 
\Delta\textrm{UV}_\psi &=& \textrm{TODO} 
\end{eqnarray}
