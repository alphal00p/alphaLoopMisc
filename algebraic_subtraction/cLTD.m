...
(* ::Input::Initialization:: *)
cFFCrossFreeFamilyReduce[g_Graph,OptionsPattern[{ConvertToNormalisedFormat->False,wrapEtaDefinitions->OptionValue[wrapEtaDefinitions],DEBUG->False}]]:=Module[
...
Return[{{{EsurfaceSign,Esurf}}}];,Return[EsurfaceSign(1/
If[OptionValue[wrapEtaDefinitions],
eta[Sort[Esurf]]
,
Total[Table[OSE[name],{name,Esurf}]]
]
)];
];
];

sources=Sort[GraphComputation`SourceVertexList[g]];
sinks=Sort[GraphComputation`SinkVertexList[g]];
...
If[OptionValue[wrapEtaDefinitions],
Esurf=Esurf[[1]]*eta[Sort[Esurf[[2]]]];
,
Esurf=Esurf[[1]]*Total[Table[OSE[name],{name,Esurf[[2]]}]];
];
];
allTerms=((cFFCrossFreeFamilyReduce[#[[2]],
ConvertToNormalisedFormat->OptionValue[ConvertToNormalisedFormat],
wrapEtaDefinitions->OptionValue[wrapEtaDefinitions],
DEBUG->OptionValue[DEBUG]])&)/@allContractedGraphs;
...
(* ::Input::Initialization:: *)
CrossFreeFamilyLTD[g_Graph,OptionsPattern[{ConvertToNormalisedFormat->False,DEBUG->False,Orientations->Automatic, wrapEtaDefinitions->False}]]:=Module[
...
ConvertToNormalisedFormat->OptionValue[ConvertToNormalisedFormat],DEBUG->OptionValue[DEBUG],wrapEtaDefinitions->OptionValue[wrapEtaDefinitions]
