...
| `--do-check-emr-energies-linearity` | Check that the final dot-product expression stays at most linear in each EMR energy by matching forbidden repeated-edge dot-product patterns. | Active for the dot-product route, including when the dot expression is loaded from file. Requires `--build_dot_products_form` or `--dot-expression-from-file`. The scan is restricted to virtual EMR edges, identified from `lmb_rep` by the presence of loop-basis `K(...)` dependence, so purely external EMR dot-products may appear with higher powers without triggering the check. It uses the same core count as `--n_core_for_dot_processing`, and shows a dedicated progress bar for the pattern scan. When a violation is found, the run prints the violating pattern plus one offending additive term immediately. A failed check is reported but does not abort the run, and `Ctrl-C` during the scan marks the check as interrupted while still continuing with evaluator construction using the partial checked-pattern count gathered so far. |
