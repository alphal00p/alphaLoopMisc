...
import math
import logging
logger = logging.getLogger('EsurfaceIntersectionFinder')
...
class EsurfaceIntersectionFinder(object):
    ...
    def __init__(self, E_surfaces, cvxpy_coordinates, E_cm, seed_point_shifts=None, debug=False, frozen_momenta=None, **opts):
        ...
        if frozen_momenta is not None:
            self.frozen_momenta = {}
            self.frozen_momenta['in'] = [
                list(v[1:]) for v in frozen_momenta['in']]
            self.frozen_momenta['out'] = [
                list(v[1:]) for v in frozen_momenta['out']]
        else:
            self.frozen_momenta = None

        self.n_frozen_momenta = 0 if self.frozen_momenta is None else len(
            self.frozen_momenta['out'])

        # Assign a truly random seed to cvxpy problem solving
        for cvxpy_coord in self.cvxpy_coordinates:
            cvxpy_coord.value = [numpy.float64(
                random.random()*self.E_cm) for i in range(0, 3)]
        ...
        for i_lop in range(0, len(self.cvxpy_coordinates)):
            ...
            for i_comp in range(0, 3):
                ...
                direction.append(((r-1.)*2.*self.E_cm if r <
                                 0. else (r+1.)*2.*self.E_cm))
            self.cvxpy_directions.append(cvxpy.Constant(direction))

        if seed_point_shifts is None or isinstance(seed_point_shifts, int):
            n_shifts = seed_point_shifts if isinstance(
                seed_point_shifts, int) else 10
            self.seed_point_shifts = [
                [
                    [random.random()*(self.E_cm/100.)*(10*((i_shift+1)/float(n_shifts//2)))
            ...
            self.seed_point_shifts += [
                [
                    [random.random()*(self.E_cm/100.)*(200*((i_shift+1) /
                                                            float(n_shifts-(n_shifts//2)))) for i_comp in range(3)]
                    for i_loop in range(len(self.cvxpy_coordinates))
    ...
    def project_onto_E_surface_closest(self, point, direction, orientations=('plus', 'minus')):
        ...
        if self.debug:
            logger.info("Now projecting point x=\n%s\nusing normal=\n%s" %
                        (str(start_point), direction))

        plus_point = [v + 2.*self.E_cm*normalized_direction[i_loop]
                      for i_loop, v in enumerate(start_point)]
        minus_point = [v - 2.*self.E_cm*normalized_direction[i_loop]
                       for i_loop, v in enumerate(start_point)]
        cvxpy_directions = {
            'plus': {
        ...
        cvxpy_directions = {k: v for k,
                            v in cvxpy_directions.items() if k in orientations}

        for orientation in cvxpy_directions:
            objective = cvxpy.norm(cvxpy.hstack([cvxpy.Constant(direction)-self.cvxpy_coordinates[i_dir]
                                   for i_dir, direction in enumerate(cvxpy_directions[orientation]['direction'])]), 2)
            ...
            cvxpy_directions[orientation]['solution'] = [Vector([float(c.value[0]), float(
                c.value[1]), float(c.value[2])]) for c in self.cvxpy_coordinates]
            cvxpy_directions[orientation]['distance'] = sum(
                [(s-v).square() for s, v in zip(cvxpy_directions[orientation]['solution'], start_point)])
            cvxpy_directions[orientation]['relative_distance'] = cvxpy_directions[orientation]['distance'] / \
                cvxpy_directions[orientation]['starting_distance']

        all_res = sorted(cvxpy_directions.items(),
                         key=lambda s: s[1]['distance'])

        if self.debug:
            logger.info("Solutions obtained:\n%s\nuReturning:%s" %
                        (pformat(cvxpy_directions), all_res[0][0]))
    ...
    def improve_seed_point_directions_per_signature(self, seed_point, maximal_cvxpy_seed_point_improvement_steps, include_push_away_soft=False):
        ...
        all_loop_sigs = list(set(
            [osp['loop_sig'] for E_surf in self.E_surfaces for osp in E_surf['onshell_propagators']]))

        # Now add all possible combinations, from more likely to yield a solution to less likely
        all_possible_loop_signatures_directions = list(
            itertools.combinations_with_replacement(base_directions, len(all_loop_sigs)))
        if len(all_possible_loop_signatures_directions) < maximal_cvxpy_seed_point_improvement_steps:
            for additional_combinations in itertools.combinations_with_replacement(base_directions+extra_directions, len(all_loop_sigs)):
                if additional_combinations not in all_possible_loop_signatures_directions:
                    all_possible_loop_signatures_directions.append(
        ...
        for signatures_directions in all_possible_loop_signatures_directions:
            ...
            for i_loop in range(0, len(self.cvxpy_coordinates)):
                if not any(sig[i_loop] != 0 for sig in all_loop_sigs):
                    new_seed_point[i_loop] = [random.random()*(self.E_cm/2.)
                                              for _ in range(0, 3)]

            direction_for_signatures = {
                s: signatures_directions[i] for i, s in enumerate(all_loop_sigs)}

            solution_sum = sum(abs(self.E_surface(
                new_seed_point, E_surf['onshell_propagators'], E_surf['E_shift'])) for E_surf in self.E_surfaces)
            ...
            if consistency_test < self.consistency_check_threshold:
                if self.debug:
                    logger.info(
                        'Current solution satisfactory enough, returning now.')
                solved = True
            ...
            if include_push_away_soft:
                objective += sum(cvxpy.norm(coord, 2)
                                 for coord in self.E_surfaces for coord in self.cvxpy_coordinates)
            ...
            new_seed_point = [Vector([float(c.value[0]), float(
                c.value[1]), float(c.value[2])]) for c in self.cvxpy_coordinates]
                ...
                for cardinal_direction in [(i, j) for j in range(0, 3) for i in range(0, len(self.cvxpy_coordinates))]:
                    ...
                    if self.debug:
                        logger.info("Step %d of improvement on the seed point\nx=%s\n that evaluates E-surface to:\n%s\nwith score %.4e by projecting onto cardinal direction %s." % (
                            ...
                            abs(solution_sum/self.E_cm),
                            'component #%d, loop #%d' % (
                                cardinal_direction[0], cardinal_direction[1])
                        ))
                    ...
                    new_seed_point = [[pi+random.random()*self.E_cm for pi in p]
                                      for p in new_seed_point]
                    ...
                    new_seed_point = self.project_onto_E_surface_closest(
                        new_seed_point, cardinal_direction, orientations=('plus',))
        ...
        while (not solved) and n_steps <= self.maximal_cvxpy_seed_point_improvement_steps:
            ...
            for i_E_surf, E_surf in enumerate(self.E_surfaces):
                ...
                E_surf_normal = [Vector([
                    self.dE_surface(new_seed_point, self.E_surfaces[i_E_surf]['onshell_propagators'],
                                    self.E_surfaces[i_E_surf]['E_shift'], loop_index, component_index)
                    for component_index in range(0, 3)
                ]) for loop_index in range(0, len(self.cvxpy_coordinates))
                ]

                new_seed_point = self.project_onto_E_surface_closest(
                    new_seed_point, E_surf_normal, orientations=('plus', 'minus'))
    ...
    def find_intersection(self):
        ...
        if self.debug:
            logger.info(
                "The attempt at finding a seed in the interior of all surfaces with cvxpy yielded:\n%s" % str(p))

        if p.status == cvxpy.OPTIMAL:
            seed_point = [[float(c.value[0]), float(c.value[1]), float(
                c.value[2])] for c in self.cvxpy_coordinates]

        if seed_point is None:
            # No possible intersection
            if self.debug:
                logger.info(
                    "Could not find a seed point in the interior of all E-surfaces. There is therefore no intersection.")
            return None
        ...
        if self.maximal_cvxpy_seed_point_improvement_steps is not None and self.maximal_cvxpy_seed_point_improvement_steps > 0:
            ...
            seed_point, solved = self.improve_seed_point_directions_per_signature(
                ...
                include_push_away_soft=(
                    self.maximal_cvxpy_seed_point_improvement_steps > 0)
            )
            ...
            if solved and (self.maximal_cvxpy_seed_point_improvement_steps < 0):
                if self.debug:
                    logger.info("The improvement of the seed point already lead to a solution for the intersection:\nx=%s\nfor which the E-surfaces evaluate as follows:\n%s" % (
                        str(seed_point),
                        '\n'.join('E-surface with id %d : %.16e' % (
        ...
        for i_shift, seed_point_shift in enumerate(self.seed_point_shifts):
            ...
            shifted_seed = [[seed_comp+seed_point_shift[i_vec][i_comp] for i_comp,
                             seed_comp in enumerate(seed_vec)] for i_vec, seed_vec in enumerate(seed_point)]

            if self.frozen_momenta is not None:
                shifted_seed[-len(self.frozen_momenta['out'])                             :] = self.frozen_momenta['out']

            if self.debug:
                logger.info("An intersection is possible, scipy.optimize.root will now attempt to find an intersection from the scipy seed point with shift #%d: %s." % (
                    i_shift, str(shifted_seed)))
            ...
            if self.use_scipy_root:
                ...
                if self.debug:
                    logger.info("The attempt at finding an intersection with scipy.optimize.root for seed shift #%d yielded:\n%s" % (
                        i_shift, str(scipy_res)))

                # logger.debug(str(scipy_res))
                if not scipy_res.success:
                    if self.debug:
                        logger.info(
                            "Could not find an intersection with scipy.optimize.root for seed shift #%d" % i_shift)
                else:
                    # pprint(self.intersection_function(scipy_res.x))
                    # pprint(self.intersection_function_jac(scipy_res.x))
                    intersection_point = [scipy_res.x[i:i+3]
                                          for i in range(0, len(scipy_res.x), 3)]
                    if self.frozen_momenta is not None:
                        intersection_point += self.frozen_momenta['out']
                    # Double-check the solution
                    solution_sum = sum(abs(self.E_surface(
                        intersection_point, E_surf['onshell_propagators'], E_surf['E_shift'])) for E_surf in self.E_surfaces)
                    consistency_test = abs(solution_sum/self.E_cm)
                    if consistency_test > self.consistency_check_threshold:
                        ...
                        if self.debug:
                            logger.info("The E-surface equations evaluate as follows for the intersection point found using shift #%d x=%s\n%s" % (
                                i_shift,
                                intersection_point,
                                '\n'.join('E-surface with id %d : %.16e' % (
            ...
            if self.use_scipy_fsolve:
                ...
                if self.debug:
                    logger.info("scipy.optimize.fsolve will now attempt to find an intersection from the seed point with shift #%d: %s." % (
                        i_shift, str(scipy_seed)))
                ...
                if not fsolve_success:
                    if self.debug:
                        logger.info(
                            "Could not find an intersection with scipy.optimize.fsolve for seed shift #%d" % i_shift)
                else:
                    # pprint(self.intersection_function(scipy_res.x))
                    # pprint(self.intersection_function_jac(scipy_res.x))
                    intersection_point = [scipy_res[i:i+3]
                                          for i in range(0, len(scipy_res), 3)]
                    if self.frozen_momenta is not None:
                        intersection_point += self.frozen_momenta['out']
                    # Double-check the solution
                    solution_sum = sum(abs(self.E_surface(
            ...
            if self.use_scipy_minimize:
                ...
                if self.debug:
                    logger.info("scipy.optimize.minimize will now attempt to find an intersection from the seed point with shift #%d: %s." % (
                        i_shift, str(scipy_seed)))
                ...
                if not fsolve_success:
                    if self.debug:
                        logger.info(
                            "Could not find an intersection with scipy.optimize.minimize for seed shift #%d" % i_shift)
                else:
                    # pprint(self.intersection_function(scipy_res.x))
                    # pprint(self.intersection_function_jac(scipy_res.x))
                    intersection_point = [scipy_res.x[i:i+3]
        ...
        if self.maximal_cvxpy_find_intersection_steps is not None and self.maximal_cvxpy_find_intersection_steps > 0:
            ...
            if solved:
                if self.debug:
                    logger.info("The second pass of cvxpy wtihout push away soft found a solution for the intersection:\nx=%s\nfor which the E-surfaces evaluate as follows:\n%s" % (
                        str(seed_point),
                        '\n'.join('E-surface with id %d : %.16e' % (
                            E_surf['id'], self.E_surface(seed_point, E_surf['onshell_propagators'], E_surf['E_shift'])) for E_surf in self.E_surfaces
                        )
        ...
        if self.debug:
            logger.info("Could not find an intersection with scipy.optimize with %d attempts of shifting the seed point." % len(
                self.seed_point_shifts))
    ...
    def delta(cls, loop_momenta, loop_sig, shift, m_squared):
        ...
        k = [sum(l[i]*factor for l, factor in zip(loop_momenta, loop_sig))
             for i in range(0, 3)]
    ...
    def ddelta(cls, loop_momenta, loop_sig, shift, m_squared, loop_index, component_index):
        ...
        k_derived = loop_momenta[loop_index][component_index] * \
            loop_sig[loop_index]

        if k_derived == 0.:
            return 0.

        k_not_derived = sum(l[component_index]*factor for i_loop_mom, (l, factor)
                            in enumerate(zip(loop_momenta, loop_sig)) if i_loop_mom != loop_index)
    ...
    def intersection_function(self, xs):
        ...
        for E_surf in self.E_surfaces:
            res.append(self.E_surface(
                loop_momenta, E_surf['onshell_propagators'], E_surf['E_shift']))
    ...
    def intersection_function_jac(self, xs):
        ...
        for E_surf in self.E_surfaces:
            ...
            for loop_index in range(0, len(loop_momenta)-self.n_frozen_momenta):
                for component_index in range(0, 3):
                    res[-1].append(self.dE_surface(loop_momenta, E_surf['onshell_propagators'],
                                   E_surf['E_shift'], loop_index, component_index))
    ...
    def intersection_scalar_function(self, xs):
        ...
        res = 0.
        for E_surf in self.E_surfaces:
            res += self.E_surface(loop_momenta,
                                  E_surf['onshell_propagators'], E_surf['E_shift'])**2
                        ...
                        for E_surf in self.E_surfaces:
                            if (loop_index_A, component_index_A) == (loop_index_B, component_index_B):
                                res[-1][-1] += (2.*(self.dE_surface(loop_momenta, E_surf['onshell_propagators'], E_surf['E_shift'], loop_index_A, component_index_A)**2) +
                                                ...
                                                self.ddE_surface(
                                                    loop_momenta, E_surf['onshell_propagators'], E_surf['E_shift'], loop_index_A, component_index_A, loop_index_A, component_index_A)
                                                )
                            else:
                                res[-1][-1] += (
                                    2.*self.dE_surface(loop_momenta, E_surf['onshell_propagators'], E_surf['E_shift'], loop_index_A, component_index_A)*self.dE_surface(
                                        loop_momenta, E_surf['onshell_propagators'], E_surf['E_shift'], loop_index_B, component_index_B)
                                    + 2.*self.E_surface(loop_momenta, E_surf['onshell_propagators'], E_surf['E_shift'])*self.ddE_surface(
                                        loop_momenta, E_surf['onshell_propagators'], E_surf['E_shift'], loop_index_A, component_index_A, loop_index_B, component_index_B)
                                )
